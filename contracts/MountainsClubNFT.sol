...
contract MountainsClubNFT is Ownable, ERC721A, PaymentSplitter {
    ...
    uint public saleStartTime1; // presale mint starting time
    uint public saleStartTime2;  //whitelisted mint starting time



    mapping(address => uint) public amountNFTsperWalletSpecialEvent; //all the minting users can mint only one time in special evernt 
    mapping(address => uint) public amountNFTsperWalletWhitelistSale; //all whitelisted users can mint 2 times in white list sale event
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
		...
		//check if the user is listed in whitelist
        require(isWhiteListed(msg.sender, _proof), "Not whitelisted"); 
